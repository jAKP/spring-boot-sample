...
public class PostController {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "return all posts by page") })
	public ResponseEntity<Page<PostDetails>> getAllPosts(@RequestParam(value = "q", required = false) String keyword, //
	...
	public ResponseEntity<Void> createPost(@RequestBody @Valid PostForm post, HttpServletRequest request) {
		...
		URI loacationHeader = ServletUriComponentsBuilder.fromContextPath(request).path("/api/posts/{id}")
	...
	public ResponseEntity<Page<CommentDetails>> getCommentsOfPost(@PathVariable("id") Long id,
			@PageableDefault(page = 0, size = 10, sort = "createdDate", direction = Direction.DESC) Pageable page) {
	...
	public ResponseEntity<Void> createCommentOfPost(@PathVariable("id") Long id, @RequestBody CommentForm comment) {
